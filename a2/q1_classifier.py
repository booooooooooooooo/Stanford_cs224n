...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        self.w = tf.Variable(tf.zeros((self.config.n_features, self.config.n_classes)))
        self.b = tf.Variable(tf.zeros((self.config.n_classes, ))) #TODO
        z = tf.add(tf.matmul(self.input_placeholder, self.w) , self.b)
        pred = softmax(z)
        ### END YOUR CODE
        return pred
