...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        oneHotToDense = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(oneHotToDense, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        xavierInit = xavier_weight_init()

        w = tf.Variable(xavierInit((n_features * embed_size, hidden_size)))
        b1 = tf.Variable(xavierInit((hidden_size,) ) )
        u = tf.Variable(xavierInit((hidden_size, n_classes)))
        b2 = tf.Variable(xavierInit( (n_classes,) ))
