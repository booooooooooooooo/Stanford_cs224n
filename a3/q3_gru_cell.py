...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_z = tf.get_variable("U_z", initializer = xavier_initializer, shape = (self.input_size, self.state_size))
            W_z = tf.get_variable("W_z", initializer = xavier_initializer, shape = (self.state_size, self.state_size))
            b_z = tf.get_variable("b_z", initializer = xavier_initializer, shape = (self.state_size, ))

            U_r = tf.get_variable("U_r", initializer = xavier_initializer, shape = (self.input_size, self.state_size))
            W_r = tf.get_variable("W_r", initializer = xavier_initializer, shape = (self.state_size, self.state_size))
            b_r = tf.get_variable("b_r", initializer = xavier_initializer, shape = (self.state_size, ))

            U_o = tf.get_variable("U_o", initializer = xavier_initializer, shape = (self.input_size, self.state_size))
            W_o = tf.get_variable("W_o", initializer = xavier_initializer, shape = (self.state_size, self.state_size))
            b_o = tf.get_variable("b_o", initializer = xavier_initializer, shape = (self.state_size, ))
            ...
            new_state_prime = tf.nn.tanh( tf.matmul(inputs, U_o) + tf.matmul( r_t * state, W_o) + b_o )
            new_state = z_t * state + (1 - z_t) * new_state_prime
            ### END YOUR CODE ###
